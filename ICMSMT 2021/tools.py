...
if _my_thread_model==2: openMdb(pathName='zamok.cae')
if _my_thread_model==3: openMdb(pathName='nkt.cae')
...
'40ХН':Material(E=210000.0e+6,mu=0.28,st=735.0e+6,sv=882.0e+6,delta=10.0,psi=45.0),
'SAE4140':Material(E=201000.0e+6,mu=0.33,st=965.0e+6,sv=1076.0e+6,delta=11.0,psi=40.0)
}
...
def createCut(Part,Sketch,Begin,P,Fi,Len,X,Y,dx,dy,dp=0.0):
    ...
    #delta=-0.00 #приріст кроку +0.008 добре,-0.008 погано
    while i*P<=Len:#довжина різьби
        ...
        if type(dp)!=list:
            s.move(objectList=s.geometry.values(),vector=(X+dx*P*tan(Fi*pi/180)*i, Y+P*i*(dy+dp/P) )) #Y+dy*P*i+delta
        else:
            s.move(objectList=s.geometry.values(),vector=(X+dx*P*tan(Fi*pi/180)*i, Y+P*i*dy+dp[i] ))
        model.parts[Part].Cut(sketch=s)
        del s
        i=i+1
        #delta+=dp #-0.008 добре,+0.008 погано
    return i-1
...
def createMesh():
    ...
    model.rootAssembly.seedEdgeBySize(constraint=FINER, deviationFactor=0.1, edges=sm.edges+ss.edges, minSizeFactor=0.1, size=0.2)
...
def createJobSubmit():
    ...
    myJob.setValues(numCpus=4, numDomains=4, numGPUs=1)
    myJob.submit()
...
def readODB():
    #openOdb(r'C:/3/fesafeRes.odb').steps['fe-safe_01'].frames[0].fieldOutputs['LOGLife-Repeats'].getSubset(position=ELEMENT_NODAL, region=openOdb(r'C:/3/fesafeRes.odb').rootAssembly.nodeSets['FIRST']).values[0] # ???
    session.Path(name='Path-1', type=NODE_LIST, expression=(('PART-1-1', (136, 783, 122, 115, 681, 647, 94, 87, 80, 73, 477, 59, 52, )), )) 
    dat=session.XYDataFromPath(name='XYData-1', path=session.paths['Path-1'] , includeIntersections=False, projectOntoMesh=False, pathStyle=PATH_POINTS, numIntervals=10, projectionTolerance=0, shape=UNDEFORMED, labelType=SEQ_ID)
    d=[x[1] for x in dat.data]
...
def runFeSafe(input_odb,input_stlx,output_odb):
    ...
    Якщо з FEA моделі імпортуються напруження і деформації,
    то перед створенням файлу sltx в налаштуваннях Analysis Options необхідно вибрати Read strains from FE Models.
    '''
    import subprocess
    s=r'c:\Program Files\Safe_Technology\fe-safe\version.6.5\exe\fe-safe_cl.exe -s j=c:\3\{iodb}.odb b=c:\3\{istlx}.stlx o=c:\3\{oodb}.odb'
