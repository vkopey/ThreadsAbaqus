...
mat1=matlib['SAE4140'].power(8)#матеріал 1
mat2=matlib['SAE4140'].power(8)#матеріал 2
bolt_load=-0.25
load1=-100.0
load2=-350.0e+6 #-1.0e+6/(pi*(d['D'].v/1000)**2-pi*(d['d3'].v/1000)**2)#-155.1e+6 #або в нютонах
...
def createProfile():
    ...
    createCut(Part='Part-2',Sketch='Sketch-4',Begin=0,P=d['P'].v,Fi=d['fi'].v,Len=d['l4'].v-16,X=d['dsr'].v,Y=d['l3'].v-15.875,dx=-1,dy=-1,dp=list(my_iter))
...
runFeSafe('Model-1','my','fesafeRes') # виконати fe-safe
myOdb = openOdb(path='fesafeRes.odb')
session.viewports['Viewport: 1'].setValues(displayedObject=session.odbs['C:/3/fesafeRes.odb']) 
my_res=readODB()
myOdb.close()
